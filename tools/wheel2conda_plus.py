...
def build(
    ...
    def make_tar_zst(base: Path, members: list[str]) -> bytes:
        raw = io.BytesIO()
    ...
    pkg_blob = make_tar_zst(stage, files)
    info_blob = make_tar_zst(infodir, info_members)

    with zipfile.ZipFile(out, "w", zipfile.ZIP_STORED) as z:
        z.writestr("metadata.json", json.dumps({"conda_pkg_format_version": 2}))
        z.writestr(f"pkg-{stem}.tar.zst", pkg_blob)
        z.writestr(f"info-{stem}.tar.zst", info_blob)
